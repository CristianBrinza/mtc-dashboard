...
export default function SMM() {
  ...
  // For basic filtering you can still use these (or remove if using only dynamic filters)
  ...
  // Dynamic multi-value filter states
  ...
  // Temporary states for adding filters
  ...
  const openMediaViewer = (images: string[], clickedImage: string) => {
    const sortedImages = [clickedImage, ...images.filter(img => img !== clickedImage)];
    setSelectedMedia(sortedImages);
    setShowMediaViewer(true);
  };

  const closeMediaViewer = (e: React.MouseEvent<HTMLDivElement>) => {
  ...
  useEffect(() => {
    axios
        .get(`${import.meta.env.VITE_BACKEND}/api/social-accounts`)
        .then((resp) => setAccounts(resp.data))
        .catch((err) => console.error("Error fetching accounts:", err));

    axios
        .get(`${import.meta.env.VITE_BACKEND}/api/types`)
        .then((resp) => setTypes(resp.data))
        .catch((err) => console.error("Error fetching types:", err));
  }, []);

  // Fetch posts when filters, sort, or pagination change
  useEffect(() => {
    if (!user) return;
    fetchPosts();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    // Use the dynamic filter arrays here
    selectedAccounts,
    selectedTypes,
    selectedDays,
    selectedPlatforms,
  ...
  const fetchPosts = async () => {
    if (!user || !["admin", "smm"].some(role => user.roles.includes(role))) {
      setErrorMessage("Unauthorized or insufficient privileges.");
      return;
    }
    setIsLoading(true);
    setErrorMessage("");
    ...
    try {
      ...
      // Send dynamic filters as arrays if set
      if (selectedAccounts.length > 0) params.accounts = selectedAccounts;
      if (selectedTypes.length > 0) params.types = selectedTypes;
      if (selectedDays.length > 0) params.days = selectedDays;
      ...
      if (selectedSubCategories.length > 0) params.sub_categories = selectedSubCategories;

      // (Optional) Also include singular filters if needed
      ...
      console.error("Error fetching SmmPosts:", error);
      setErrorMessage(error.response?.data?.error || "Failed to load SMM Posts");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "SMM Posts");
    XLSX.writeFile(workbook, "SMM_Posts.xlsx");
  };
                ...
                posts.map((post) => (
                    <div className={styles.smmPostsTable_costume} key={post._id}>
                      <div className={styles.smmPostsTable_costume_left}>
                        ...
                        <div
                            ...
                            onClick={() =>
                                post.images && openMediaViewer(post.images, post.images[0])
                            }
                        >
                          {post.images && post.images.length > 0 ? (
                              <img
                                  src={`${import.meta.env.VITE_BACKEND}${post.images[0]}`}
                                  alt="Post media"
                        ...
                        <div className={styles.smmPostsTable_costume_platform}>
                          {post.sponsored ? (
                              <div className={styles.smmPostsTable_costume_sponsored_active}>
                                $
                              </div>
                          ) : (
                              <div className={styles.smmPostsTable_costume_sponsored}></div>
                          )}
                        </div>
                        <div className={styles.smmPostsTable_costume_account}>
                          {post.account}
                        </div>
                    ...
                    <span className={styles.smmPostsTable_costume_date_day}>
                      ...
                      {post.hour}
                        ...
                        <div className={styles.smmPostsTable_costume_type}>
                          {post.type}
                        </div>
                        <div className={styles.smmPostsTable_costume_tags}>
                          {post.tags && post.tags.length > 0 ? (
                              post.tags.map((tag, index) => (
                                  <div key={index} className={styles.smmPostTag}>
                                    {tag}
                                  </div>
                              ))
                          ) : (
                              <div className={styles.smmPostTagEmpty}></div>
                          )}
                        </div>
                      </div>
                      <div className={styles.smmPostsTable_costume_right}>
                        ...
                        <div className={styles.smmPostsTable_costume_category}>
                          {post.category}
                        </div>
                        <div className={styles.smmPostsTable_costume_category}>
                          {post.sub_category}
